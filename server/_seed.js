...
function restructureData(dataArray) {
  return dataArray
    ...
    .reduce((acc, val) => 
      [...acc, ...Object.keys(val).map(key => ({ ':name': key, ...val[key] }))]
    , []);
}
...
function getShowTitle(dateStr) {
  ...
  const title = `${localDatetimeString}${showPosition ? ` (${showPosition})` : ''}`
  return title;
}
  ...
  return dataArr
    .map(data => {
      ...
      const newSets = sets.map((set, setIterator) => {
        ...
        const newSongInstances = songInstances.map((songInstance, songInstanceIterator) => {
