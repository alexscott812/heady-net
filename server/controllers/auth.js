...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ msg: 'Password reset functionality not yet available' });
  } catch (err) {
    return next(createError(500, err.message));
  }
};
...
const resetPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return next(createError(400, `No user found with password reset token ${token}`));
    }

    const newPasswordMatch = req.body.new_password === req.body.confirm_new_password;
    if (!newPasswordMatch) {
