...
const getUserById = async (req, res, next) => {
  try {
    ...
    if (!user[0]) {
      return next(
        createError(404, `No user found with ID of ${req.params.id}`)
      );
    }
...
const addUser = async (req, res, next) => {
  try {
    ...
    const { password, password_reset_token, __v, ...passwordlessNewUser } =
      user;
...
const updateUserById = async (req, res, next) => {
  try {
    ...
    const { password, password_reset_token, __v, ...passwordlessUser } = user;
  ...
  try {
    ...
    if (user._id !== req.user._id) {
      return next(
        createError(
          403,
          `Not allowed to update user with ID of ${req.params.id}`
        )
      );
    }
