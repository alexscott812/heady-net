...
const gdId = "6faa7ca7-0d99-4a5e-bfa6-1fd5037520c6";
const bsId = "640db492-34c4-47df-be14-96e2cd4b9fe4";
const baseUrl = `https://api.setlist.fm/rest/1.0/artist/${bsId}/setlists`;
...
function tranformSetlistObject(obj) {
  const [day, month, year] = obj.eventDate.split("-").map((x) => parseInt(x));
...
function getVenues(data, { cities, states, countries }) {
  const uniqueVenues = getUniqueObjsByName(
    data.map((x) => ({
      ...x.venue,
...
function getSongs(data) {
  const uniqueSongs = getUniqueObjsByName(
    data
      .map((x) =>
        x.sets.map((set) =>
          set.song_instances.map((songInstance) => songInstance.song)
        )
      )
      .flat(3)
  );
...
async function main() {
  ...
  const shows = getShows(reformattedData, {
    cities,
    states,
    countries,
    venues,
  ...
  console.log(JSON.stringify(shows.slice(0, 10)));
}
