      ...
      <ModalContent>
        ...
        <ModalBody py={0}>
          {showsData && !hasNoShowsData && (
            <Box mb={4}>
              <Text variant="subtle-bold" mb={2}>
                Shows
                <Badge ml={1} colorScheme="brand">
                  {showsMeta.total_results}
                </Badge>
              </Text>
              <Stack spacing={2}>
                {showsData.map((show) => (
          ...
          {shouldSearch &&
            ...
            hasNoVenuesData &&
            hasNoSongsData &&
            hasNoUsersData && <EmptyState mb={4} />}
        </ModalBody>
      </ModalContent>
    </Modal>
  );
