...
const addVenue = async ({ venue, tokenFn }) => {
	try {
		...
		const { data } = await axiosInstance.post('/venues', venue, {
			headers: {
...
const updateVenue = async ({ venue, tokenFn }) => {
	try {
		const token = await tokenFn();
		const { data } = await axiosInstance.put(`/venues/${venue._id}`, venue, {
			headers: {
				Authorization: `Bearer ${token}`
			}
		});
...
export { getVenues, getVenueById, addVenue, updateVenue, deleteVenue };
